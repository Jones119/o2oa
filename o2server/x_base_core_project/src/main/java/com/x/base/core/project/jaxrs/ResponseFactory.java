...
public class ResponseFactory {
	...
	public static <T> Response getDefaultActionResultResponse(ActionResult<T> result) {
		if (result.getType().equals(ActionResult.Type.error)) {
			...
			return switch (result.getData()) {
				case WoFile wo -> Response.ok(wo.getBytes()).header(Content_Disposition, wo.getContentDisposition())
				...
				case WoSeeOther wo -> {
					try {
						yield Response.seeOther(new URI(wo.getUrl())).build();
					} catch (Exception e) {
				...
				default -> Response.ok(result.toJson()).cacheControl(defaultCacheControl).build();
			};
		}
	}
		...
		if (result.getType().equals(ActionResult.Type.error)) {
			...
			return switch (result.getData()) {
				case WoFile wo -> {
					...
					if (null != maxAge) {
						...
						if (wo.getStreamingOutput() != null) {
							yield Response.ok(wo.getStreamingOutput())
							...
							yield Response.ok(wo.getBytes()).header(Content_Disposition, wo.getContentDisposition())
									.header(Content_Type, wo.getContentType()).header(Content_Length, wo.getBytes().length)
									.header(Accept_Ranges, "none").cacheControl(cacheControl)
									.lastModified(wo.getLastModified()).build();
						}
				...
				case WoText wo -> {
					...
					yield Response.ok(wo.getText()).type(wo.getContentType()).tag(tag).build();
				}
				case WoContentType wo -> {
				...
				default -> {
					...
					if (null != maxAge) {
						CacheControl cacheControl = new CacheControl();
						cacheControl.setMaxAge(maxAge);
						yield Response.ok(result.toJson()).cacheControl(cacheControl).build();
					} else {
						EntityTag tag = new EntityTag(etagDefault(result.getData()));
						if (notModified(request, tag)) {
							yield Response.notModified().tag(tag).build();
						}
						yield Response.ok(result.toJson()).tag(tag).build();
					}
				}
			};
		}
	...
	private static Integer maxAgeDefault(Object o) {
		return switch (o) {
			case WoMaxAgeFastETag fast -> fast.getMaxAge();
			default -> null;
		};
	}
