...
setsid ${current_dir}/jvm/sw_java25/bin/java -javaagent:${current_dir}/console.jar -server -Djava.awt.headless=true -Xms4g -Xmx4g -Duser.timezone=GMT+08 -XX:+HeapDumpOnOutOfMemoryError -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.authenticate=false -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=0.0.0.0:20000 -Djava.rmi.server.hostname=127.0.0.1 -XX:+UnlockExperimentalVMOptions -XX:+EnableJVMCI --module-path=${current_dir}/commons/module_java25 --upgrade-module-path=${current_dir}/commons/module_java25/compiler.jar:${current_dir}/commons/module_java25/compiler-management.jar -jar ${current_dir}/console.jar
